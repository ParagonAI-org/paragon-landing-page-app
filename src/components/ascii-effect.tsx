...
void mainImage(const in vec4 inputColor, const in vec2 uv, out vec4 outputColor) {
  ...
  if (noiseIntensity > 0.0) {
    float noiseVal = noise(workUV * noiseScale + currentTime * noiseSpeed);
  ...
  if (jitterIntensity > 0.0) {
    float jitterTime = currentTime * jitterSpeed;
  ...
  if (glitchIntensity > 0.0 && glitchFrequency > 0.0) {
    float glitchTime = floor(currentTime * glitchFrequency);
...
type AsciiStyleName = 'standard' | 'dense' | 'minimal' | 'blocks'
...
class AsciiEffectImpl extends Effect {
  constructor(options: AsciiEffectOptions) {
    ...
    const uniforms = new Map<string, Uniform<number | boolean | Vector2>>([
      ['cellSize', new Uniform(cellSize)],
      ['invert', new Uniform(invert)],
      ['colorMode', new Uniform(color)],
      ['asciiStyle', new Uniform(style)],
      ['time', new Uniform(0)],
      ['resolution', new Uniform(new Vector2().copy(resolution))],
...
export const AsciiEffect = forwardRef<AsciiEffectImpl, AsciiEffectProps>(
  function AsciiEffect(
    ...
    const styleMap: Record<AsciiStyleName, number> = {
      standard: 0,
      dense: 1,
      minimal: 2,
      blocks: 3,
