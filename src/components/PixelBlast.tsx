...
const PixelBlast = ({
  ...
  // biome-ignore lint/suspicious/noExplicitAny: three.js internals
      ...
      if (noiseAmount > 0) {
        ...
        const noiseEffect = new Effect(
          ...
          `uniform float uTime; uniform float uAmount;
           ...
           void mainImage(const in vec4 inputColor,const in vec2 uv,out vec4 outputColor){
             float n=hash(floor(uv*vec2(1920.,1080.))+floor(uTime*60.));
             float g=(n-0.5)*uAmount;
             outputColor=inputColor+vec4(vec3(g),0.);
           }`,
          {
            uniforms: new Map([
              ['uTime', new THREE.Uniform(0)],
        ...
        if (composer?.passes?.length > 0)
          composer.passes.forEach(
            (p: { renderToScreen: boolean }) => (p.renderToScreen = false),
          )
